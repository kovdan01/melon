...
std::shared_ptr<sqlpp::mysql::connection_config> config_db()
{
    ...
    config->user     = (std::getenv("DB_USER")     == nullptr ? "melon"      : std::getenv("DB_USER"));
    config->password = (std::getenv("DB_PASSWORD") == nullptr ? "melonpass"  : std::getenv("DB_PASSWORD"));
    config->database = (std::getenv("DB_DATABASE") == nullptr ? "melon"      : std::getenv("DB_DATABASE"));
    config->host     = (std::getenv("DB_HOST")     == nullptr ? "localhost"  : std::getenv("DB_HOST"));
    config->port     = (std::getenv("DB_PORT")     == nullptr ? default_port : std::stoi(std::getenv("DB_PORT")));
#ifndef NDEBUG
    config->debug = true;
#else
    config->debug = false;
