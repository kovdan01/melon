...
id_t max_message_id(sqlpp::mysql::connection& db, id_t chat_id, id_t domain_id)
{
    ...
    auto result = db(select(max(G_MESSAGES.messageId)).from(G_MESSAGES).where(G_MESSAGES.chatId == chat_id && G_MESSAGES.domainId == domain_id));
    if (!result.empty())
    {
        const auto& row = result.front();
        return row.max;
...
Message::Message(sqlpp::mysql::connection& db, std::uint64_t chat_id, std::uint64_t domain_id, std::uint64_t user_id,
     ...
     : mc::Message(chat_id, domain_id, user_id, std::move(text), timestamp, status)
     , m_db(db)
{
    set_message_id(max_message_id(db, chat_id, domain_id) + 1);
...
Message::Message(sqlpp::mysql::connection& db, std::uint64_t message_id, std::uint64_t chat_id, std::uint64_t domain_id)
     ...
     , m_db(db)
{
    auto result = db(select(G_MESSAGES.userId, G_MESSAGES.text, G_MESSAGES.status, G_MESSAGES.sendtime)
